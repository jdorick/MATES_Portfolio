# Blitting is a rendering method for 2D games that allows for improved performance
...
window_width = 600
window_height = 500 # The height is 1 pixel greater because any more breaks the graphics
...
background_image = 'images/jason.jpg' # Picture of Mr. Kelsey
...
def flappygame():
	...
	pipeVelX = -8 # Doubled pipe velocity

	# bird velocity
	bird_velocity_y = -9
	bird_Max_Vel_Y = 10
	bird_Min_Vel_Y = +8
	birdAccY = -1 # Made negative to swap gravity

	bird_flap_velocity = 8 # Made positive to swap gravity
	bird_flapped = False
	while True:
		...
		window.blit(game_images['jason'], (0, 0))
		for upperPipe, lowerPipe in zip(up_pipes, down_pipes):
...
#If the vertical of the bird is within the vertical of the pipe or of the game menu, then you lose
...
def createPipe(): 
	offset = window_height/ (random.randint(15, 50) / 10) # Random height of the pipes
	pipeHeight = game_images['pipeimage'][0].get_height()
	y2 = offset + \
		random.randrange(
...
if __name__ == "__main__":
	...
	game_images['jason'] = pygame.image.load(
